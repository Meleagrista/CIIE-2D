...
class Bar(pygame.sprite.Sprite):
    def __init__(self, screen: pygame.Surface):
        ...
        self._x = screen.get_height() * 0.02
        self._y = screen.get_height() * 0.02
        self._percentage = 0

        self._sprite_sheet = SpriteSheet(UI_ASSETS, 20, 11, 80, 80)
    ...
    def draw(self, **kwargs):
        ...
        tile_id = int(round(self._percentage * 6 + 1)) + 20*5

        tile = self._sprite_sheet.get_sprite_by_number(tile_id)
        surface.blit(tile, (self._x - offset.x, self._y - offset.y))
